function ydata = tsne_p_opt(P, labels, no_dims)
...
%     H = zeros(2*n);
    ...
    for iter=1:max_iter
        ...
        num(1:n, 1:n) = 1 ./ (1 + bsxfun(@plus, sum_ydata, bsxfun(@plus, sum_ydata', -2 * (ydata * ydata')))); % Student-t distribution
%         Z(1:n, 1:n) = (1 + bsxfun(@plus, sum_ydata, bsxfun(@plus, sum_ydata', -2 * (ydata * ydata')))); % Student-t distribution
%         num(1:n, 1:n) = 1 ./ Z;
        num(1:n+1: end) = 0;                                        % set diagonal to zero
        Q(1:n, 1:n) = max(num ./ sum(sum(num)), realmin);          % normalize to get probabilities

        % Compute the gradients (faster implementation)
        L(1:n, 1:n) = (P - Q) .* num;
        y_grads(1:n, 1:no_dims) = 4 * (diag(sum(L, 1)) - L) * ydata;
    
        % Compute the hessian
%         deltay = (ones(ydata)'*ydata)' - (ones(ydata)'*ydata);
%         % now i need to convert P and Q to 4X4 for each const. 
%         H(1:2*n, 1:2*n) = 4*(P - Q) .* (-Z - deltay(:)*deltay(:)') * (num.*num);
        ...
        if ~rem(iter, 10)
            ...
            last_toc = toc
            txt_iter = sprintf('Iteration %g out of %g: : error is: %g toc:%g', iter, max_iter,cost, last_toc);
            disp(txt_iter);
